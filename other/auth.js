...
(function () {
    ...
    <div class="ssm-auth-modal" role="dialog" aria-modal="true" aria-label="लॉगिन एवं प्रोफ़ाइल">
      ...
      <div id="ssmAuthLoggedOut">
        <div class="ssm-auth-header">
          ...
          <p class="ssm-auth-tagline">अपने खाते में प्रवेश करें अथवा नया खाता बनाएँ</p>
        </div>

        <div class="ssm-auth-tabs">
          <button class="ssm-tab active" data-tab="login" type="button">लॉगिन</button>
          <button class="ssm-tab" data-tab="signup" type="button">साइन अप</button>
        </div>

        <form id="ssmLoginForm" class="ssm-auth-form">
          <label for="ssmLoginEmail">ईमेल</label>
          <input type="email" id="ssmLoginEmail" required autocomplete="email">
          <label for="ssmLoginPassword">पासवर्ड</label>
          <input type="password" id="ssmLoginPassword" required autocomplete="current-password">
          <button type="submit" class="ssm-btn-primary">लॉगिन करें</button>
          <p class="ssm-auth-error" id="ssmLoginError"></p>
        </form>

        <form id="ssmSignupForm" class="ssm-auth-form" hidden>
          <label for="ssmSignupName">पूरा नाम</label>
          <input type="text" id="ssmSignupName" required autocomplete="name">
          <label for="ssmSignupEmail">ईमेल</label>
          <input type="email" id="ssmSignupEmail" required autocomplete="email">
          <label for="ssmSignupPhone">फ़ोन नंबर</label>
          <input type="tel" id="ssmSignupPhone" autocomplete="tel">
          <label for="ssmSignupAddress">पता</label>
          <input type="text" id="ssmSignupAddress" autocomplete="address-line1">
          <label for="ssmSignupPassword">पासवर्ड (कम से कम 6 अक्षर)</label>
          <input type="password" id="ssmSignupPassword" required minlength="6" autocomplete="new-password">
          <button type="submit" class="ssm-btn-primary">खाता बनाएँ</button>
          <p class="ssm-auth-error" id="ssmSignupError"></p>
        </form>
      </div>
  ...
  function init() {
    ...
    // साइडबार / ड्रॉअर में जो भी लिंक class="ssm-login-trigger" रखते हैं, उन सब पर क्लिक जोड़ें
    document.querySelectorAll(".ssm-login-trigger").forEach(trigger => {
      trigger.addEventListener("click", (e) => { e.preventDefault(); openModal(); });
    });

    /* ---------- टैब स्विच (लॉगिन / साइन अप) ---------- */
    const tabs = document.querySelectorAll(".ssm-tab");
    const loginForm = document.getElementById("ssmLoginForm");
    const signupForm = document.getElementById("ssmSignupForm");
    tabs.forEach(tab => {
    ...
    signupForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const errorEl = document.getElementById("ssmSignupError");
      errorEl.textContent = "";
      const name = document.getElementById("ssmSignupName").value.trim();
      const email = document.getElementById("ssmSignupEmail").value.trim();
      const phone = document.getElementById("ssmSignupPhone").value.trim();
      const address = document.getElementById("ssmSignupAddress").value.trim();
      const password = document.getElementById("ssmSignupPassword").value;

      try {
        const cred = await auth.createUserWithEmailAndPassword(email, password);
        await cred.user.updateProfile({ displayName: name });
        // ---- यूज़र का डेटा "users" कलेक्शन में, uid के नाम से एक डॉक्यूमेंट में सहेजा गया ----
    ...
    loginForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const errorEl = document.getElementById("ssmLoginError");
      errorEl.textContent = "";
      const email = document.getElementById("ssmLoginEmail").value.trim();
      const password = document.getElementById("ssmLoginPassword").value;
      try {
    ...
    function friendlyError(err) {
      const map = {
        "auth/email-already-in-use": "यह ईमेल पहले से एक खाते से जुड़ा है।",
        "auth/invalid-email": "कृपया सही ईमेल दर्ज करें।",
